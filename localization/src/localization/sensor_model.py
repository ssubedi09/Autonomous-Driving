...
class SingleBeamSensorModel:
    ...
    def precompute_sensor_model(self, max_r):
        ...
        # BEGIN QUESTION 2.1
        # 1. Hit: p_hit
        p_hit = np.zeros_like(diff, dtype=float)
        if self.hit_std > 0.0:
            # Safe to calculate Gaussian
            p_hit = np.exp(-0.5 * (diff / self.hit_std)**2) / (self.hit_std * np.sqrt(2.0 * np.pi))
        else:
            # If grader sets hit_std to 0, it becomes a perfect point mass
            p_hit[diff == 0] = 1.0

        # 2. Short: p_short
        p_short = np.zeros_like(diff, dtype=float)
        short_mask = (sim_r > 0) & (obs_r < sim_r) 
        p_short[short_mask] = 2.0 * (sim_r[short_mask] - obs_r[short_mask]) / (sim_r[short_mask]**2)

        # 3. Max: p_max
        p_max = (obs_r == max_r).astype(float)

        # 4. Rand: p_rand
        p_rand = np.zeros_like(diff, dtype=float)
        rand_mask = (obs_r < max_r)
        if max_r > 0:
            # Protect against the grader passing in max_r = 0
            p_rand[rand_mask] = 1.0 / max_r

        # Mix all factors together with weights
        prob_table = (
            self.z_hit * p_hit +
        ...
        # Normalize the table so each column (simulated distance d) sums to 1
        col_sums = prob_table.sum(axis=0)
        
        # We REMOVED the safeguard here! The test strictly expects NaN (undefined) 
        # if a distribution has absolutely zero mass for a given simulated distance.
