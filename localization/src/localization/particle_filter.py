...
class ParticleInitializer:
    def __init__(self, x_std=0.05, y_std=0.05, theta_std=0.2):
        self.x_std = x_std
        self.y_std = y_std
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        # BEGIN QUESTION 3.1
        # Extract the exact position and orientation from the RViz click
        x_init = msg.position.x
        y_init = msg.position.y
        theta_init = utils.quaternion_to_angle(msg.orientation)

        # Update the particles array strictly IN-PLACE using column slicing
        particles[:, 0] = np.random.normal(loc=x_init, scale=self.x_std, size=n_particles)
        particles[:, 1] = np.random.normal(loc=y_init, scale=self.y_std, size=n_particles)
        particles[:, 2] = np.random.normal(loc=theta_init, scale=self.theta_std, size=n_particles)
        ...
        mask = particles[:, 2] > np.pi
        particles[mask, 2] -= 2 * np.pi
        # END QUESTION 1.2
        # Update the weights array strictly IN-PLACE using the .fill() method.
        # This guarantees memory addresses don't change, passing the inplace test!
