...
def compute_position_in_frame(p, frame):
    ...
    """
    # BEGIN QUESTION 1.2
    # 1. Translate the point relative to the new frame's origin
    dx = p[0] - frame[0]
    dy = p[1] - frame[1]
    
    # 2. Extract the heading of the new frame
    theta = frame[2]
    
    # 3. Rotate the translated point by -theta using a 2D rotation matrix
    cos_t = np.cos(theta)
    sin_t = np.sin(theta)
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            # BEGIN QUESTION 1.1
            # 1. Compute Euclidean distances from the current pose to all path waypoints
            dx = path_xytv[:, 0] - pose[0]
            dy = path_xytv[:, 1] - pose[1]
            distances = np.sqrt(dx**2 + dy**2)
            
            # 2. Find the index of the closest waypoint on the path
            closest_idx = np.argmin(distances)
            
            # 3. Consider only the waypoints from the closest one onwards
            forward_distances = distances[closest_idx:]
            
            # 4. Find where the distance exceeds the lookahead
            exceed_mask = forward_distances > distance_lookahead
            
            if not np.any(exceed_mask):
                # If no points exceed the lookahead distance, we've reached the end
                return len(path_xytv) - 1
            
            # np.argmax on a boolean array returns the index of the first True value
            first_exceed_rel_idx = np.argmax(exceed_mask)
            target_idx = closest_idx + first_exceed_rel_idx
            
            # 5. Look back one index to see which state is actually closer to the exact lookahead
            if target_idx > closest_idx:
                dist_ahead = distances[target_idx]
                dist_behind = distances[target_idx - 1]
                
                # Compare absolute differences to see which is closer to the target distance
                if abs(dist_behind - distance_lookahead) < abs(dist_ahead - distance_lookahead):
                    return target_idx - 1
