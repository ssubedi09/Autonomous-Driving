...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        # BEGIN QUESTION 4.1
        # Create an array of K steering angles evenly spaced between min and max delta
        deltas = np.linspace(self.min_delta, self.max_delta, self.K)
        
        # Broadcast the steering angles into the controls array
        # deltas[:, None] reshapes to (K, 1) so it broadcasts across the T dimension
        controls[:, :, 1] = deltas[:, None]
        # END QUESTION 4.1
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        # BEGIN QUESTION 4.2
        # Iterate over the T timesteps
        for t in range(self.T):
            # Compute changes for all K sequences at step t
            # rollouts[:, t, :] has shape (K, 3)
            # controls[:, t, :] has shape (K, 2)
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        # the reference state
        # BEGIN QUESTION 4.3
        # Extract the final x, y position for each of the K rollouts
        final_positions = rollouts[:, -1, :2]
        
        # Extract the reference x, y position
        ref_position = reference_xyt[:2]
        
        # Calculate the Euclidean distance (L2 norm) for each rollout
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # BEGIN QUESTION 4.3
        # Reshape rollouts to (K * (T + 1), 3) to pass into check_collisions_in_map
        flat_rollouts = rollouts.reshape(-1, 3)
        
        # Get collisions (returns array of shape K * (T + 1))
        collisions = self.check_collisions_in_map(flat_rollouts)
        
        # Reshape collisions back to (K, T + 1)
        reshaped_collisions = collisions.reshape(self.K, self.T + 1)
        
        # Count the number of collisions in each rollout (sum along the time axis)
        collision_counts = np.sum(reshaped_collisions, axis=1)
        
        # Multiply by the collision weight
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        control = self.sampled_controls[best_index, 0, :]
        return control
        # END QUESTION 4.4
