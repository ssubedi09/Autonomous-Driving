    ...
    while len(queue) > 0:
        ...
        for neighbor, w in rm.graph[entry.node].items():
            ...
            if not expanded[neighbor]:
                new_cost_to_come = entry.cost_to_come + weight
                f_value = new_cost_to_come + h
                queue.push(QueueEntry(f_value, next(c), neighbor, entry.node, new_cost_to_come))
            # END QUESTION 2.1
...
def shortcut(rm, vpath, num_trials=100):
    ...
    for _ in range(num_trials):
        ...
        direct_length = rm.heuristic(u, v)
        path_length = rm.compute_path_length(vpath[i:j+1])
        
        # Only collision-check if the direct path is actually an improvement
        if direct_length < path_length:
            if rm.check_edge_validity(u, v):
                # Keep everything up to 'i', and everything from 'j' onwards
