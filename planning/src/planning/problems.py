...
class PlanarProblem(object):
    ...
    def check_state_validity(self, states):
        ...
        # BEGIN QUESTION 1.2
        # Extract the new coordinates since utils.world_to_map modified them in place
        map_x = states[:, 0].astype(int)
        map_y = states[:, 1].astype(int)
        
        # Only check the grid for points that are already valid (within bounds).
        # This prevents out-of-bounds IndexError.
        # Remember: images/arrays are indexed as [row (y), col (x)]
        valid[valid] = self.permissible_region[map_y[valid], map_x[valid]]
        # END QUESTION 1.2
