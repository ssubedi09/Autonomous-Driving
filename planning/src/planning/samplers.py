...
class HaltonSampler(Sampler):
    ...
    def sample(self, num_samples):
        ...
        # BEGIN QUESTION 1.1
        # Extract lower and upper bounds across all dimensions
        lower_bounds = np.array(self.extents)[:, 0]
        upper_bounds = np.array(self.extents)[:, 1]
        
        # Scale: samples * range + offset
        return batch * (upper_bounds - lower_bounds) + lower_bounds
        # END QUESTION 1.1
